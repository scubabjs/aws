...
def Main():
    my_parser = argparse.ArgumentParser(
        prog='firehose2s3', description='Use Firehouse to import data to S3')
    my_parser.add_argument('bucket', type=str,
                           help='specify S3 bucket to push data into')
    my_parser.add_argument('firehose', type=str,
                           help='specify name of the the firehose')

    """Exercise Kinesis Firehose methods"""

    # Assign these values before running the program
    # If the specified IAM role does not exist, it will be created
    firehose_name = args.firehose
    bucket_arn = 'arn:aws:s3:::' + args.bucket
    ...
    if not firehose_exists(firehose_name):
        ...
        firehose_arn = create_firehose_to_s3(
            firehose_name, bucket_arn, iam_role_name)
...
def get_firehose_arn(firehose_name):
    ...
    firehose_client = boto3.client('firehose')
    try:
        result = firehose_client.describe_delivery_stream(
...
def wait_for_active_firehose(firehose_name):
    ...
    while True:
        ...
        if status == 'DELETING':
            logging.error(
                f'Firehose delivery stream {firehose_name} is being deleted.')
            return False
        time.sleep(2)
